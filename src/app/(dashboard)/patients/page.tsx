...
import { Patient, Doctor, QueryParams, PaginatedResponse } from "@/types";
...
export default function PatientsPage() {
  ...
  const { data, loading, error, refetch } = useFetch<PaginatedResponse<Patient>>(
    (p) => patientApi.getAll(p as QueryParams).then((r) => r.data),
  ...
  const renderBody = () => {
    ...
    return patients.map((p) => (
      <tr key={p._id} className="hover:bg-gray-50 transition-colors">
        ...
        <td className="px-4 py-3 text-gray-600">{typeof p.doctor === "object" ? p.doctor.name : "—"}</td>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>{["Name","Age","Gender","Condition","Doctor","Phone","Actions"].map(h => (
